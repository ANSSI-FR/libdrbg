...
#if defined(SIMPLE_AES) || defined(TABLE_AES)
/**** Common helpers ************/
/* Memcpy helper */
USED_ATTR static inline void local_copy(void *dst, const void *src, uint32_t n)
{
...
/* Memset helper */
USED_ATTR static inline void local_set(void *dst, uint8_t val, uint32_t n)
{
	uint8_t *ldst = (uint8_t*)dst;
	uint32_t i;
...
int aes_setkey_dec(aes_core_context *ctx, const uint8_t *key, uint32_t keybits)
{
	...
	return aes_setkey_enc(ctx, key, keybits);
}
...
int aes_enc(aes_core_context *ctx, const uint8_t data_in[16], uint8_t data_out[16])
{
	...
	/* Clean stuff */
	local_set(&X0, 0, sizeof(X0)); local_set(&X1, 0, sizeof(X1));
	local_set(&X2, 0, sizeof(X2)); local_set(&X3, 0, sizeof(X3));
	/* */
	local_set(&Y0, 0, sizeof(Y0)); local_set(&Y1, 0, sizeof(Y1));
	local_set(&Y2, 0, sizeof(Y2)); local_set(&Y3, 0, sizeof(Y3));
...
int aes_setkey_dec(aes_core_context *ctx, const uint8_t *key, uint32_t keybits)
{
	...
	/* Perform encryption key schedule */
	if(aes_setkey_enc(&ctxx, key, keybits)){
		goto err;
	}
	ctx->nr = ctxx.nr;

	/* Now modify the context accordingly to decryption 
	 * by reversing the key order.
	 */
	SK = (uint32_t*)(ctxx.rk + (4 * ctxx.nr));
	(*RK) = (*SK); RK++; SK++;
	(*RK) = (*SK); RK++; SK++;
	(*RK) = (*SK); RK++; SK++;
	(*RK) = (*SK); RK++; SK++;
	for(i = (ctx->nr - 1), SK -= 8; i > 0; i--){
		for(j = 0; j < 4; j++){
			*RK++ = Td0[ sbox[ ( (*SK)       ) & 0xFF ] ] ^
				Td1[ sbox[ ( (*SK) >>  8 ) & 0xFF ] ] ^
				Td2[ sbox[ ( (*SK) >> 16 ) & 0xFF ] ] ^
...
int aes_dec(aes_core_context *ctx, const uint8_t data_in[16], uint8_t data_out[16])
{
	...
	/* Last round without Mixcolumns */
	X0 = (*RK) ^
	    ( (uint32_t) inv_sbox[ ( Y0	      ) & 0xFF ]       ) ^
	    ( (uint32_t) inv_sbox[ ( Y3 >>  8 ) & 0xFF ] <<  8 ) ^
	    ( (uint32_t) inv_sbox[ ( Y2 >> 16 ) & 0xFF ] << 16 ) ^
	    ( (uint32_t) inv_sbox[ ( Y1 >> 24 ) & 0xFF ] << 24 ); RK++;

	X1 = (*RK) ^
	    ( (uint32_t) inv_sbox[ ( Y1       ) & 0xFF ]       ) ^
	    ( (uint32_t) inv_sbox[ ( Y0 >>  8 ) & 0xFF ] <<  8 ) ^
	    ( (uint32_t) inv_sbox[ ( Y3 >> 16 ) & 0xFF ] << 16 ) ^
	    ( (uint32_t) inv_sbox[ ( Y2 >> 24 ) & 0xFF ] << 24 ); RK++;

	X2 = (*RK) ^
	   ( (uint32_t) inv_sbox[ ( Y2       ) & 0xFF ]       ) ^
	   ( (uint32_t) inv_sbox[ ( Y1 >>  8 ) & 0xFF ] <<  8 ) ^
	   ( (uint32_t) inv_sbox[ ( Y0 >> 16 ) & 0xFF ] << 16 ) ^
	   ( (uint32_t) inv_sbox[ ( Y3 >> 24 ) & 0xFF ] << 24 ); RK++;

	X3 = (*RK) ^
	   ( (uint32_t) inv_sbox[ ( Y3       ) & 0xFF ]       ) ^
	   ( (uint32_t) inv_sbox[ ( Y2 >>  8 ) & 0xFF ] <<  8 ) ^
	   ( (uint32_t) inv_sbox[ ( Y1 >> 16 ) & 0xFF ] << 16 ) ^
	   ( (uint32_t) inv_sbox[ ( Y0 >> 24 ) & 0xFF ] << 24 ); RK++;
