...
static const uint32_t C1_MD4[13] = {
	0, 4, 8, 12, 0, 1, 2, 3, 3, 7, 11, 19, 0
};
static const uint32_t C2_MD4[13] = {
	0, 1, 2, 3, 0, 4, 8, 12, 3, 5, 9, 13, 0x5a827999
};
static const uint32_t C3_MD4[13] = {
...
static inline int md4_process(md4_context *ctx,
			   const uint8_t data[MD4_BLOCK_SIZE])
{
	uint32_t A, B, C, D;
	uint32_t W[16];
	uint32_t idx;
...
int md4_update(md4_context *ctx, const uint8_t *input, uint32_t ilen)
{
	...
	fill = (uint16_t)(MD4_BLOCK_SIZE - left);

	ctx->md4_total += ilen;

	if ((left > 0) && (remain_ilen >= fill)) {
		/* Copy data at the end of the buffer */
		memcpy(ctx->md4_buffer + left, data_ptr, fill);
	...
	if (remain_ilen > 0) {
		memcpy(ctx->md4_buffer + left, data_ptr, remain_ilen);
	}
...
int md4_final(md4_context *ctx, uint8_t output[MD4_DIGEST_SIZE])
{
	...
	uint8_t last_padded_block[2 * MD4_BLOCK_SIZE];
	int ret;
	...
	if (block_present != 0) {
		...
		memcpy(last_padded_block, ctx->md4_buffer,
			     block_present);
	}
	...
	if (block_present > (MD4_BLOCK_SIZE - 1 - sizeof(uint64_t))) {
		/* We need an additional block */
		PUT_UINT64_LE(8 * ctx->md4_total, last_padded_block,
			      (2 * MD4_BLOCK_SIZE) - sizeof(uint64_t));
		...
		PUT_UINT64_LE(8 * ctx->md4_total, last_padded_block,
			      MD4_BLOCK_SIZE - sizeof(uint64_t));
		ret = md4_process(ctx, last_padded_block); EG(ret, err);
	}
