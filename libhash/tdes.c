...
static inline void des_ip(uint32_t L[1], uint32_t R[1])
{
	uint32_t T;
...
static inline void des_fp(uint32_t L[1], uint32_t R[1])
{
	uint32_t T;
...
static inline void des_round(uint32_t L[1], uint32_t R[1], uint64_t K)
{
	uint32_t T;
	uint32_t k1, k2;
...
int des_set_key(des_context *ctx, const uint8_t k[8], des_direction dir)
{
	...
	for( i = 0; i < 16; i++ ){
		...
		if(dir == DES_ENCRYPTION){
			ctx->sk[i] = (((uint64_t)k2) << 32) | (uint64_t)k1;
		}
		else if(dir == DES_DECRYPTION){
			ctx->sk[15-i] = (((uint64_t)k2) << 32) | (uint64_t)k1;
		}
		else{
			ret = -1;
			goto err;
...
/* DES encryption core */
static inline int des_core(const des_context *ctx, const uint8_t input[8], uint8_t output[8])
{
	uint32_t L, R;
	uint32_t i;
	int ret;
 ...
 */
typedef int dummy;
#endif /* defined(WITH_BC_TDEA) || (WITH_HASH_MDC2) */
