...
static inline int md5_process(md5_context *ctx,
			   const uint8_t data[MD5_BLOCK_SIZE])
{
...
int md5_update(md5_context *ctx, const uint8_t *input, uint32_t ilen)
{
	...
	fill = (uint16_t)(MD5_BLOCK_SIZE - left);

	ctx->md5_total += ilen;

	if ((left > 0) && (remain_ilen >= fill)) {
		/* Copy data at the end of the buffer */
		memcpy(ctx->md5_buffer + left, data_ptr, fill);
	...
	if (remain_ilen > 0) {
		memcpy(ctx->md5_buffer + left, data_ptr, remain_ilen);
	}
...
int md5_final(md5_context *ctx, uint8_t output[MD5_DIGEST_SIZE])
{
	...
	uint8_t last_padded_block[2 * MD5_BLOCK_SIZE];
	int ret;
	...
	if (block_present != 0) {
		...
		memcpy(last_padded_block, ctx->md5_buffer,
			     block_present);
	}
	...
	if (block_present > (MD5_BLOCK_SIZE - 1 - sizeof(uint64_t))) {
		/* We need an additional block */
		PUT_UINT64_LE(8 * ctx->md5_total, last_padded_block,
			      (2 * MD5_BLOCK_SIZE) - sizeof(uint64_t));
		...
		PUT_UINT64_LE(8 * ctx->md5_total, last_padded_block,
			      MD5_BLOCK_SIZE - sizeof(uint64_t));
		ret = md5_process(ctx, last_padded_block); EG(ret, err);
	}
