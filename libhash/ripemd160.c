...
#define ROTL_RIPEMD160(x, n)    ((((uint32_t)(x)) << (n)) | (((uint32_t)(x)) >> (32-(n))))
...
/* Left constants */
static const uint32_t KL_RIPEMD160[5] = {
	0x00000000, 0x5a827999, 0x6ed9eba1, 0x8f1bbcdc, 0xa953fd4e
};
/* Right constants */
static const uint32_t KR_RIPEMD160[5] = {
	0x50a28be6, 0x5c4dd124, 0x6d703ef3, 0x7a6d76e9, 0x00000000
};

/* Left line */
static const uint8_t RL_RIPEMD160[5][16] = {
...
static const uint8_t SL_RIPEMD160[5][16] = {
...
/* Right line */
static const uint8_t RR_RIPEMD160[5][16] = {
...
static const uint8_t SR_RIPEMD160[5][16] = {
...
#define RIPEMD160_CORE(a, b, c, d, e, round, idx, w, F, S, R, K) do { 				\
	uint32_t t = ROTL_RIPEMD160(a + F(b, c, d) + w[R[round][idx]] + K[round], S[round][idx]) + e;\
	a = e; e = d; d = ROTL_RIPEMD160(c, 10); c = b; b = t; 					\
} while(0)

/* RIPEMD160 core processing */
static int ripemd160_process(ripemd160_context *ctx,
			   const uint8_t data[RIPEMD160_BLOCK_SIZE])
{
	/* Left line */
...
int ripemd160_update(ripemd160_context *ctx, const uint8_t *input, uint32_t ilen)
{
	...
	fill = (uint16_t)(RIPEMD160_BLOCK_SIZE - left);

	ctx->ripemd160_total += ilen;

	if ((left > 0) && (remain_ilen >= fill)) {
		/* Copy data at the end of the buffer */
		memcpy(ctx->ripemd160_buffer + left, data_ptr, fill);
	...
	if (remain_ilen > 0) {
		memcpy(ctx->ripemd160_buffer + left, data_ptr, remain_ilen);
	}
...
int ripemd160_final(ripemd160_context *ctx, uint8_t output[RIPEMD160_DIGEST_SIZE])
{
	...
	uint8_t last_padded_block[2 * RIPEMD160_BLOCK_SIZE];
	int ret;
	...
	if (block_present != 0) {
		...
		memcpy(last_padded_block, ctx->ripemd160_buffer,
			     block_present);
	}
	...
	if (block_present > (RIPEMD160_BLOCK_SIZE - 1 - sizeof(uint64_t))) {
		/* We need an additional block */
		PUT_UINT64_LE(8 * ctx->ripemd160_total, last_padded_block,
			      (2 * RIPEMD160_BLOCK_SIZE) - sizeof(uint64_t));
		...
		PUT_UINT64_LE(8 * ctx->ripemd160_total, last_padded_block,
			      RIPEMD160_BLOCK_SIZE - sizeof(uint64_t));
		ret = ripemd160_process(ctx, last_padded_block); EG(ret, err);
	}
