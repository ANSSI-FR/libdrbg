...
typedef struct streebog_context_ {
	uint8_t streebog_digest_size;
	uint8_t streebog_block_size;
	streebog_endianness streebog_endian;
	/* Number of bytes processed */
	uint64_t streebog_total;
	/* STREEBOG state */
	uint64_t h[STREEBOG_BLOCK_U64_SIZE];
	uint64_t N[STREEBOG_BLOCK_U64_SIZE];
	uint64_t Sigma[STREEBOG_BLOCK_U64_SIZE];
	/* Internal buffer to handle updates in a block */
	uint8_t streebog_buffer[STREEBOG_BLOCK_SIZE];
...
/* STREEBOG constants*/
static const uint64_t C_STREEBOG[12][STREEBOG_BLOCK_U64_SIZE] =
{
     { /* 0 */
...
static const uint64_t PI[STREEBOG_BLOCK_U64_SIZE][256] = {
	{ /* 0 */
...
/* Extract byte i from uint64_t and shift it right by 8*j */
#define B(x, i, j) ((uint64_t)(((const uint8_t*)&(x))[(i)]) << (8*(j)))
...
static inline uint64_t streebog_permute(const uint64_t in[STREEBOG_BLOCK_U64_SIZE], uint8_t i)
{
	uint64_t t = 0;
	unsigned int j;
...
static inline void streebog_transform(uint64_t out[STREEBOG_BLOCK_U64_SIZE],
				      const uint64_t a[STREEBOG_BLOCK_U64_SIZE],
				      const uint64_t b[STREEBOG_BLOCK_U64_SIZE])
{
	uint64_t tmp[STREEBOG_BLOCK_U64_SIZE];
	unsigned int j;
	...
	for(j = 0; j < STREEBOG_BLOCK_U64_SIZE; j++){
		out[j] = streebog_permute(tmp, (uint8_t)j);
	}

	return;
}

static inline void gN(uint64_t h[STREEBOG_BLOCK_U64_SIZE],
		      const uint64_t m[STREEBOG_BLOCK_U64_SIZE],
		      const uint64_t N[STREEBOG_BLOCK_U64_SIZE])
{
	uint64_t K[STREEBOG_BLOCK_U64_SIZE];
	uint64_t T[STREEBOG_BLOCK_U64_SIZE];
	unsigned int j;
...
static inline void streebog_process(streebog_context *ctx, const uint8_t *in, uint64_t num)
{
	uint64_t M[STREEBOG_BLOCK_U64_SIZE];
	uint64_t l, CF;
	unsigned int j;
