...
static int sha224_process(sha224_context *ctx,
			   const uint8_t data[SHA224_BLOCK_SIZE])
{
...
int sha224_update(sha224_context *ctx, const uint8_t *input, uint32_t ilen)
{
	...
	fill = (uint16_t)(SHA224_BLOCK_SIZE - left);

	ctx->sha224_total += ilen;

	if ((left > 0) && (remain_ilen >= fill)) {
		/* Copy data at the end of the buffer */
		memcpy(ctx->sha224_buffer + left, data_ptr, fill);
	...
	if (remain_ilen > 0) {
		memcpy(ctx->sha224_buffer + left, data_ptr, remain_ilen);
	}
...
int sha224_final(sha224_context *ctx, uint8_t output[SHA224_DIGEST_SIZE])
{
	...
	uint8_t last_padded_block[2 * SHA224_BLOCK_SIZE];
	int ret;
	...
	if (block_present != 0) {
		...
		memcpy(last_padded_block, ctx->sha224_buffer,
			     block_present);
	}
	...
	if (block_present > (SHA224_BLOCK_SIZE - 1 - sizeof(uint64_t))) {
		/* We need an additional block */
		PUT_UINT64_BE(8 * ctx->sha224_total, last_padded_block,
			      (2 * SHA224_BLOCK_SIZE) - sizeof(uint64_t));
		...
		PUT_UINT64_BE(8 * ctx->sha224_total, last_padded_block,
			      SHA224_BLOCK_SIZE - sizeof(uint64_t));
		ret = sha224_process(ctx, last_padded_block); EG(ret, err);
	}
