...
#define _BASH_ROTHI_(x, y) (((x) << (y)) | ((x) >> ((sizeof(uint64_t) * 8) - (y))))
 ...
 */
#define BASH_ROTHI(x, y) ((((y) < (sizeof(uint64_t) * 8)) && ((y) > 0)) ? (_BASH_ROTHI_(x, y)) : (x))
...
#define BASH_PERMUTE(S) do {							\
	uint64_t S_[BASH_SLICES_X * BASH_SLICES_Y];				\
...
static const uint64_t bash_rc[BASH_ROUNDS] =
{
...
static const uint8_t bash_rot[BASH_ROT_ROUNDS][BASH_ROT_IDX] =
{
	{  8, 53, 14,  1 },
	{ 56, 51, 34,  7 },
	{  8, 37, 46, 49 },
...
typedef struct bash_context_ {
	uint8_t bash_digest_size;
	uint8_t bash_block_size;
	bash_endianness bash_endian;
	/* Local counter */
	uint64_t bash_total;
	/* Bash state */
	uint64_t bash_state[BASH_SLICES_X * BASH_SLICES_Y];
	/* Initialization magic value */
	uint64_t magic;
} bash_context;

int _bash_init(bash_context *ctx, uint8_t digest_size);
int _bash_update(bash_context *ctx, const uint8_t *buf, uint32_t buflen);
